...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        SalePortal = request.env['portal.sales.proposal']
    ...
    def portal_my_proposal(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        return request.render("portal_sales_proposal.my_portal_proposal", values)
    ...
    def portal_proposal_page(self, proposal_id, report_type=None, access_token=None, message=False, download=False, **kw):
        try:
            order_sudo = self._document_check_access('portal.sales.proposal', proposal_id, access_token=access_token)
        except (AccessError, MissingError):
            return request.redirect('/my')

        if report_type in ('html', 'pdf', 'text'):
            return self._show_report(model=order_sudo, report_type=report_type,
                                     report_ref='portal_sales_proposal.action_report_portal_sales_proposal', download=download)
        ...
        return request.render('portal_sales_proposal.portal_proposal_template', values)
    ...
    def update_proposals_line_dict(self, line_id, remove=False, proposal_id=None, access_token=None,price=False,**kwargs):
        ...
        if order_sudo.state not in ('draft', 'sent'):
            return False
        order_line = request.env['portal.sales.proposal.line'].sudo().browse(int(line_id))
