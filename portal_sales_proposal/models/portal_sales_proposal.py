...
class PortalSalesProposal(models.Model):
    ...
    def create(self, vals):
        ...
        if vals.get('name', _('New')) == _('New'):
            ...
            vals['name'] = self.env['ir.sequence'].next_by_code('portal.sales.proposal', sequence_date=seq_date) or _(
                'New')
        ...
        result = super(PortalSalesProposal, self).create(vals)
        return result
                ...
                for tax in line.tax_id:
                    ...
                    for t in taxes:
                        if t['id'] == tax.id or t['id'] in tax.children_tax_ids.ids:
                            ...
                            res[group]['base'] += t['base']
            res = sorted(res.items(), key=lambda l: l[0].sequence)
            order.amount_by_group_accepted = [(l[0].name, l[1]['amount'], l[1]['base'],
                                               fmt(l[1]['amount']), fmt(l[1]['base']),
                                               len(res),)for l in res]
    ...
    def onchange_partner_id(self):
        """
        Update pricelist when the partner is changed
        """
    ...
    def action_send_mail(self):
        ...
        mail_template = self.env.ref('portal_sales_proposal.mail_template_portal_sales_proposal')
    ...
    def _get_share_url(self, redirect=False, signup_partner=False, pid=None):
        ...
        return super(PortalSalesProposal, self)._get_share_url(redirect, signup_partner, pid)
...
class PortalSalesProposalLine(models.Model):
    ...
    proposal_id = fields.Many2one('portal.sales.proposal', string='Proposal Reference', required=True,
